...
public class ExcelUtils {
     ...
     * @param  object
     * @return 返回数据集合
     * @throws UnicomRuntimeException
     * @throws IOException
     */
    public static List<Map<String, Object>> leading(MultipartFile file, Object object) throws UnicomRuntimeException, IOException {
        String fileName = file.getOriginalFilename();
        // 上传文件为空
        if (StringUtils.isEmpty(fileName)) {
            throw new UnicomRuntimeException(ErrorType.OPERATION_EXCEL_ERROR, "没有导入文件");
        }
        //上传文件大小为1000条数据
        if (file.getSize() > 1024 * 1024 * 10) {
            logger.error("upload | 上传失败: 文件大小超过10M，文件大小为：{}", file.getSize());
            throw new UnicomRuntimeException(ErrorType.OPERATION_EXCEL_ERROR, "上传失败: 文件大小不能超过10M!");
        }
        // 上传文件名格式不正确
        if (fileName.lastIndexOf(".") != -1 && !".xlsx".equals(fileName.substring(fileName.lastIndexOf(".")))) {
            throw new UnicomRuntimeException(ErrorType.OPERATION_EXCEL_ERROR, "文件名格式不正确, 请使用后缀名为.XLSX的文件");
        }
        //读取数据
        ...
        for (int i = 0; i < lineList.size(); i++) {
            if (null != lineList.get(i)) {
                ...
                for (int j = 0; j < fileNameList.size(); j++) {
                    ...
                    hashMap.put(fileNameList.get(j), property);
                }
